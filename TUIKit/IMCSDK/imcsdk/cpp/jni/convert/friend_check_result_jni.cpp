...
namespace tim {
    namespace jni {
        ...
        jobject FriendCheckResultJni::Convert2JObject(const json::Object &friendCheckResult_json) {
            ...
            jstring userID_jStr = StringJni::Cstring2Jstring(env, friendCheckResult_json[kTIMFriendshipCheckFriendTypeResultIdentifier]);
            if (userID_jStr) {
                env->SetObjectField(friendInfoResultObj, j_field_array_[FieldIDUserID], userID_jStr);
                env->DeleteLocalRef(userID_jStr);
            }

            env->SetIntField(friendInfoResultObj, j_field_array_[FieldIDResultCode], friendCheckResult_json[kTIMFriendshipCheckFriendTypeResultCode]);

            jstring resultInfo_jStr = StringJni::Cstring2Jstring(env, friendCheckResult_json[kTIMFriendshipCheckFriendTypeResultDesc]);
            if (resultInfo_jStr) {
            ...
            env->SetIntField(friendInfoResultObj, j_field_array_[FieldIDResultType], friendCheckResult_json[kTIMFriendshipCheckFriendTypeResultRelation]);
