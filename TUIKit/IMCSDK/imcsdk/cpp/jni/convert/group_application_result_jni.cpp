...
namespace tim {
    namespace jni {
        ...
        jobject GroupApplicationResultJni::Convert2JObject(const json::Object &applicationResult_json) {
            ...
            env->SetIntField(jObj, j_field_array_[FieldIDUnreadCount], applicationResult_json[kTIMGroupPendencyResultUnReadNum]);

            if (applicationResult_json.HasKey(kTIMGroupPendencyResultPendencyArray)){\
                json::Array pendency_array = applicationResult_json[kTIMGroupPendencyResultPendencyArray];
                for (const auto & i : pendency_array) {
                    jobject j_obj_application = GroupApplicationJni::Convert2JObject(i);
