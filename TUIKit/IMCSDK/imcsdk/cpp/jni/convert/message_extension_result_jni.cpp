...
namespace tim {
    namespace jni {
        ...
        jobject MessageExtensionResultJni::Convert2JObject(const json::Object &extensionResult_json) {
            ...
            env->SetIntField(extensionResultObj, j_field_array_[FieldIDResultCode], extensionResult_json[kTIMMsgExtensionResultCode]);

            jstring jStr;
            jStr = StringJni::Cstring2Jstring(env, extensionResult_json[kTIMMsgExtensionResultInfo]);
            if (jStr) {
                env->SetObjectField(extensionResultObj, j_field_array_[FieldIDResultInfo], jStr);
                env->DeleteLocalRef(jStr);
            }

            jobject extensionObj = MessageExtensionJni::Convert2JObject(extensionResult_json[kTIMMsgExtensionItem]);
            if (extensionObj){
