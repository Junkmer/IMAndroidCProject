...
namespace tim {
    namespace jni {
        ...
        jobject MessageReactionResultJni::Convert2JObject(const json::Object &reactionResult_json) {
            ...
            env->SetIntField(reactionResultObj, j_field_array_[FieldIDResultCode], reactionResult_json[kTIMMsgReactionResultCode]);

            jstring jStr;
            jStr = StringJni::Cstring2Jstring(env, reactionResult_json[kTIMMsgReactionResultInfo]);
            if (jStr) {
                env->SetObjectField(reactionResultObj, j_field_array_[FieldIDResultInfo], jStr);
                env->DeleteLocalRef(jStr);
            }

            jStr = StringJni::Cstring2Jstring(env, reactionResult_json[kTIMMsgReactionResultMsgId]);
            if (jStr) {
                env->SetObjectField(reactionResultObj, j_field_array_[FieldIDMessageID], jStr);
                env->DeleteLocalRef(jStr);
            }

            if (reactionResult_json.HasKey(kTIMMsgReactionResultReactionList)){
                json::Array reaction_array = reactionResult_json[kTIMMsgReactionResultReactionList];
                jobject reactionListObj = ArrayListJni::NewArrayList();
                for (const auto &item : reaction_array){
                    jobject reactionObj = MessageReactionJni::Convert2JObject(item);
                    if (reactionObj){
